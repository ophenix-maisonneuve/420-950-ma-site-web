...
Un tournant majeur survient avec la création des projets **OWASP** et la standardisation des vulnérabilités web (OWASP Top 10). Dans ce contexte, **OWASP ZAP** voit le jour à la fin des années 2000 comme *fork* de Paros, avec un objectif premier : rendre les tests DAST accessibles aux développeurs.
...
Aujourd’hui, le DAST n’est plus un outil ponctuel de tests d'intrusion, mais un **composant continu du cycle de vie logiciel**.

---

## Objectifs

L’objectif principal du DAST est d’identifier des **vulnérabilités exploitables dans des conditions réelles**. Autrement dit, on ne cherche plus à prédire des failles potentielles, mais plutôt à démontrer que des vulnérabilités sont bel et bien présentes lors de l'exécution.
...
Un élément clé du DAST est son **orientation résultat** : lorsqu’une vulnérabilité est détectée, elle est généralement accompagnée d’une preuve concrète (requête, réponse, charge utile, etc), ce qui réduit considérablement le nombre de faux positifs par rapport à d’autres approches.
...
Le DAST excelle également dans la détection de problèmes transversaux, tels que :
