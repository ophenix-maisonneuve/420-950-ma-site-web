...
Une **nomenclature logicielle** (ou *SBOM - Software Bill of Materials*) est un inventaire détaillé et structuré de tous les composants logiciels constituant une application : librairies, dépendances transitives, versions exactes, licences, métadonnées, empreintes, provenance, modules compilés, scripts auxiliaires, etc. Elle joue le même rôle que la liste d'ingrédients d'une recette, mais appliquée au logiciel.

Historiquement, les organisations n’avaient qu’une vision partielle des composants utilisés dans un projet. Aujourd’hui, avec la généralisation de l’open source et l’évolution des attaques sur la chaîne d'approvisionnement, les nomenclatures logicielles sont devenues essentielles pour :
