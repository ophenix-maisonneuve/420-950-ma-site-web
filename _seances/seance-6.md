---
layout: default
title: "2026-04-02"
nav_order: 6
published: true
---

# Séance 6 : OWASP Top 10 (partie 1)

## 2 avril 2026

### Ordre du jour

1. Informations générales
1. Introduction à [OWASP Top 10](../notes/owasp-top-10)
1. Introduction à WebGoat
1. [A10 - Mauvaise gestion des conditions d'exception](../notes/a10)
1. [A09 - Journaux et alertes défaillants](../notes/a09)
    1. Exercice individuel: *WebGoat / (A9) Security Logging Failures / Logging Security*
1. [A08 - Intégrité défaillante des logiciels / données](../notes/a08)
    1. Exercice en groupe : *WebGoat / (A8) Software & Data Integrity / Insecure Deserialization*
1. [A07 - Défaillances d'authentification](../notes/a07)
    1. Exercice en groupe : *WebGoat / (A7) Identity & Auth Failure / JWT tokens* (leçons 1 à 8)
    1. Exercice individuel : *WebGoat / (A7) Identity & Auth Failure / JWT tokens* (leçons 9 à 11)
        1. ***À noter :** Pour certains exercices, il y a un recoupement avec A01 - Broken Access Controls*
    1. Exercice individuel : *WebGoat / (A7) Identity & Auth Failure / Secure Passwords*
1. [A06 - Conception non sécurisée](../notes/a06)


### Lectures recommandées

- Notes de cours sur la [OWASP Top 10](../notes/owasp-top-10)
    - [A10 - Mauvaise gestion des conditions d'exception](../notes/a10)
    - [A09 - Journaux et alertes défaillants](../notes/a09)
    - [A08 - Intégrité défaillante des logiciels / données](../notes/a08)
    - [A07 - Défaillances d'authentification](../notes/a07)
    - [A06 - Conception non sécurisée](../notes/a06)



### Exercice(s) complémentaire(s)

- Continuer le [travail pratique 2](../travaux/tp2)
- Exercice : *WebGoat / (A7) Identity & Auth Failure / Authentication Bypasses*
- Exercice : *WebGoat / (A7) Identity & Auth Failure / Insecure Login*
- Exercice : *WebGoat / (A7) Identity & Auth Failure / Password reset*
- Exercice : *WebGoat / (A7) Identity & Auth Failure / JWT tokens*
    - À partir de la leçon 12
